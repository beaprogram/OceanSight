...
def process_video(source,output,detector,confidence=.25,max_frames=150,callback=None):
    cap=cv2.VideoCapture(str(source));writer=None;rows=[]
    try:
        ...
        for i in range(max_frames):
            ...
            h,w=frame.shape[:2]
            if max(h,w)>1280:
                scale=1280/max(h,w);frame=cv2.resize(frame,(int(w*scale)//2*2,int(h*scale)//2*2))
            ...
            if writer is None:
                ...
                if not writer.isOpened():raise RuntimeError('Could not open the annotated-video encoder')
            detections,ms=detector.predict(frame,confidence)
            annotated=annotate(frame,detections);writer.write(annotated)
            rows.append({'frame':i,'detections':len(detections),'pipeline_ms':ms})
            if callback:callback(i,annotated)
    finally:
        cap.release()
        if writer is not None:writer.release()
    if not rows:raise ValueError('The video could not be decoded')
    return rows
