...
def postprocess(output, transform, shape, confidence=.25, iou=.45):
    predictions=output[0].T
    scores=predictions[:,4]
    keep=scores>=confidence
    xywh=predictions[keep,:4];scores=scores[keep]
    ...
    for i in np.asarray(indexes).reshape(-1):
        x,y,w,h=xywh[i]
        box=[float(np.clip((x-left)/scale,0,shape[1])),float(np.clip((y-top)/scale,0,shape[0])),
             float(np.clip((x+w-left)/scale,0,shape[1])),float(np.clip((y+h-top)/scale,0,shape[0]))]
        if box[2]>box[0] and box[3]>box[1]: result.append({'box':box,'confidence':float(scores[i]),'label':'marine_debris'})
    return result
...
class Detector:
    ...
    def predict(self,image,confidence=.25):
        start=time.perf_counter();tensor,transform=preprocess(image,self.size)
...
def annotate(image,detections):
    ...
    for d in detections:
        ...
        cv2.putText(out,f"debris {d['confidence']:.2f}",(x1,max(15,y1-5)),cv2.FONT_HERSHEY_SIMPLEX,.45,(45,220,150),1,cv2.LINE_AA)
    return out
