...
def run():
    ...
    for r in records:
        ...
        h, w = im.shape[:2]
        labels = (
            (DATA / "labels" / "test" / r["file_name"].replace(".jpg", ".txt"))
            .read_text()
            .splitlines()
        )
        truth = []
        ...
        pred, ms = detector.predict(im, 0.25)
    ...
    save_json(
        REPORTS / "failure_cases.json",
        {
            "confidence": 0.25,
            "iou": 0.5,
            "backend": "custom ONNX demo pipeline",
            "totals": totals,
            "images": rows,
        },
    )
    ...
    for ax, r in zip(axes.flat, order):
        ...
        for b in r["ground_truth"]:
            ...
            cv2.rectangle(im, (x1, y1), (x2, y2), (255, 170, 50), 1)
        ax.imshow(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))
        ax.axis("off")
        ax.set_title(f"{r['file_name']}\nTP {r['tp']} / FP {r['fp']} / FN {r['fn']}", fontsize=8)
    fig.suptitle(
        "Largest error counts: green predictions, blue ground truth | confidence 0.25, IoU 0.5"
    )
    fig.tight_layout()
    fig.savefig(REPORTS / "failure_contact_sheet.jpg", dpi=140)
    plt.close(fig)
    samples = REPORTS / "samples"
    samples.mkdir(exist_ok=True)
    for r in sorted(rows, key=lambda r: r["tp"], reverse=True)[:3]:
        cv2.imwrite(
            str(samples / r["file_name"]), annotate(pictures[r["file_name"]], r["predictions"])
        )
