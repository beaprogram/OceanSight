...
def prepare(limit=720, seed=42, frames_per_video=6):
    setup()
    records = {}
    for source in ['train', 'val', 'test']:
        ...
        for row in csv.DictReader(io.StringIO(raw.decode())):
            name = row['file_name']
            group_id(name)
            rec = records.setdefault(name, {'file_name': name, 'source_split': source, 'annotations': []})
    ...
    rng.shuffle(ids)
    n = len(ids)
    split_ids = {'train': ids[:int(n*.7)], 'val': ids[int(n*.7):int(n*.85)], 'test': ids[int(n*.85):]}
    chosen = []
    for split, gids in split_ids.items():
        ...
        cap = int(limit * (.7 if split == 'train' else .15))
    ...
    def download(rec):
        ...
        return {**rec, 'width':w, 'height':h, 'sha256':hashlib.sha256(raw).hexdigest(), 'debris_boxes':len(labels)}
    ...
    for split in split_ids:
        (DATA/f'{split}.txt').write_text('\n'.join(str(DATA/'images'/split/r['file_name']) for r in manifest if r['split']==split)+'\n')
    config = {'path': str(DATA), 'train':'train.txt','val':'val.txt','test':'test.txt','names':{0:'marine_debris'}}
    (DATA/'dataset.yaml').write_text(yaml.safe_dump(config))
    summary = {'source':BASE, 'revision':REVISION, 'seed':seed, 'split_policy':'video ID disjoint, 70/15/15 groups; capped evenly spaced frames per video',
               'task':'binary detection: all trash_* labels mapped to marine_debris; animal/plant/ROV ignored',
               'frames_per_video':frames_per_video, 'available_videos':n,
               'counts':{s:{'images':sum(r['split']==s for r in manifest), 'videos':len({r['group'] for r in manifest if r['split']==s}),
                            'boxes':sum(r['debris_boxes'] for r in manifest if r['split']==s), 'negative_images':sum(r['debris_boxes']==0 and r['split']==s for r in manifest)} for s in split_ids},
               'metadata_sha256':{s:hashlib.sha256((DATA/'raw'/f'{s}.csv').read_bytes()).hexdigest() for s in split_ids},
               'limitations':['Third-party mirror; annotation conversion not verified against original archive.', 'Video-disjoint does not guarantee expedition/location-disjoint.', 'Image list comes from annotations; unannotated background frames may be absent.']}
...
if __name__ == '__main__':
    p=argparse.ArgumentParser(); p.add_argument('--limit',type=int,default=720); p.add_argument('--seed',type=int,default=42); p.add_argument('--frames-per-video',type=int,default=6)
    a=p.parse_args(); prepare(a.limit,a.seed,a.frames_per_video)
