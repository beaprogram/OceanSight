...
def features(image):
    ...
    quality = [
        gray.mean() / 255,
        gray.std() / 255,
        np.log1p(cv2.Laplacian(gray, cv2.CV_64F).var()),
        np.mean(gray < 20),
        np.mean(gray > 235),
        hsv[:, :, 1].mean() / 255,
    ]
...
def audit():
    ...
    rows = [
        {
            ...
            "brightness": float(X[i, -6]),
            "contrast": float(X[i, -5]),
            "log_sharpness": float(X[i, -4]),
        }
    ...
    ax.scatter(coords[:, 0], coords[:, 1], c=clusters, cmap="tab10", s=20, alpha=0.7)
    ax.set(
        title="Training images: color/texture/quality feature clusters",
        xlabel="PCA 1",
        ylabel="PCA 2",
    )
    fig.tight_layout()
    fig.savefig(REPORTS / "quality_clusters.png", dpi=150)
    plt.close(fig)
    fig, axes = plt.subplots(3, 4, figsize=(12, 8))
    for ax, i in zip(axes.flat, np.argsort(scores)[-12:][::-1]):
        ax.imshow(cv2.cvtColor(images[i], cv2.COLOR_BGR2RGB))
        ax.set_title(f"{records[i]['file_name']}\nscore {scores[i]:.3f}", fontsize=7)
        ax.axis("off")
    fig.tight_layout()
    fig.savefig(REPORTS / "outlier_contact_sheet.jpg", dpi=130)
    plt.close(fig)
    save_json(
        REPORTS / "audit.json",
        {
            ...
            "features": "32-bin HSV histogram + brightness, contrast, log Laplacian sharpness, dark/bright fractions, saturation",
